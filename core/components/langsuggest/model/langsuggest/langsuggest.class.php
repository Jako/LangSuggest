...
class LangSuggest
{
    ...
    public function showLangSuggest()
    {
        ...
        if (empty($contextmap)) {
            ...
            $this->modx->cacheManager->set($cacheKey, $contextmap, 0);
        }
        ...
        if ($preferredContextKey !== $this->modx->context->get('key')) {
            ...
            if ($preferredContext) {
                ...
                if (!$foreignId = $this->modx->runSnippet('BabelTranslation', array(
                    ...
                    $foreignId = ($preferredContext) ? $preferredContext->getOption('site_start') : $currentId;
                }
                ...
                // Emulate regClientHTMLBlock for the chunk code
                $search[] = '</body>';
                $replace[] = $this->getChunk($tpl, $placeholder) . "\n" . '</body>';
